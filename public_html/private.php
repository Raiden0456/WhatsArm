...
<html>
    <head>
        ...
        <script>
            ...
            function Chat(mode, id=''){
                ...
                //AJAX
                $.ajax({
                    url: 'private_ajax.php',
                    type: 'POST',
                    cache: false,
                    data: mode ? ({message: m, whereto:username, count:messagenum, deletemess:id}): ({message:'', whereto:username, count:messagenum, deletemess:''}),
                    success: function(callback) {
                        ...
                        for (key in ret) {
                            text = '';
                            text += '<strong>'+ret[key]['login']+'</strong> ';
                            ...
                            text += ret[key]['login'] == iamuser ? '<em><a id = "delete" onclick="mdelete('+ret[key]['id']+')"> Удалить сообщение</a></em><br><br>' : '<br><br>';
    ...
    <body>
        ...
        <?php if(isset($_SESSION['username'])) { ?><p>Вы вошли в приватный чат с <script>document.write(username)</script></p><?php } ?>
        <?php if (isset($_SESSION['username'])) { ?><p><a href="chatauth.php?out"><input type = "button" id = "exit" value = "Выйти"></a></p><?php } ?>
        <?php if (isset($_SESSION['username'])) { ?><p><a href="chat.php?out"><input type = "button" id = "exit" value = "Вернуться в общий чат"></a></p><?php } ?>
        <form>
            <div id = "texto">
                <p id = "out"></p>
